...
class MyVisitor(SolidityVisitor):
    ...
    def __init__(self):
        self
        self.json_data = readwritejson.loadjson()
    ...
    def visitStateVariableDeclaration(self, ctx:SolidityParser.StateVariableDeclarationContext):
        ...
        if ctx.getChildCount()>6:
            ...
            for i in range(0, ctx.getChildCount()):
                if ctx.annotatedTypeName().__eq__(ctx.getChild(i)):
                    anotindex = i
                elif ctx.identifier().__eq__(ctx.getChild(i)):
                    idfindex = i
            ...
            if ctx.getChild(ctx.getChildCount()-2).__str__().__eq__('='):
                str+= ctx.getChild(ctx.getChildCount()-3).__str__()+ctx.vist(ctx.expression())+ctx.getChild(ctx.getChildCount()-1).__str__()
            else:
        ...
        #annotated_type=annotatedTypeName idf=identifier ('=' expr=expression) ';'
            ...
            str+= ctx.getChild(0).__str__()+" "+self.visitAnnotatedTypeName(ctx.annotatedTypeName())+" "+self.visitIdentifier(ctx.identifier())+ctx.getChild(3).__str__()
        #annotated_type=annotatedTypeName idf=identifier ';'
        else:
            # json 삽입
            data = self.json_data["g_field"][0]
            if self.visitIdentifier(ctx.identifier()) == data["name"]:
                str += self.visitAnnotatedTypeName(ctx.annotatedTypeName())+"@"+data["owner"]+" "+self.visitIdentifier(ctx.identifier())+ctx.getChild(2).__str__()
            else:
    ...
    def visitParameter(self, ctx:SolidityParser.ParameterContext):
        if ctx.annotatedTypeName().__eq__(ctx.getChild(0)):
            ...
            str = self.visitAnnotatedTypeName(ctx.annotatedTypeName())
            if ctx.getChildCount()>1:
                data = self.json_data["functions"][0]
                data = data["args"][1]
                if self.visitIdentifier(ctx.identifier()) == data["name"]:
                    str+= "@"+data["owner"]
